...
/**
 * BuilderRenderer loads and renders content from Builder.io using their SDK
 * This is a more advanced version that attempts to render the content visually
 */
const BuilderRenderer: React.FC<BuilderRendererProps> = ({ 
  apiKey, 
  model, 
  entry 
}) => {
  ...
  const [builderSDKLoaded, setBuilderSDKLoaded] = useState(false);

  // Load the Builder.io SDK
  useEffect(() => {
    if (typeof window !== 'undefined' && !window.hasOwnProperty('BuilderComponent')) {
      const script = document.createElement('script');
      script.src = 'https://cdn.builder.io/js/webcomponents';
      script.async = true;
      script.onload = () => {
        setBuilderSDKLoaded(true);
      };
    ...
    } else if (typeof window !== 'undefined' && window.hasOwnProperty('BuilderComponent')) {
      setBuilderSDKLoaded(true);
    }
  }, []);
  ...
  }, [apiKey, model, entry, builderSDKLoaded]);

  // Loading state
  if (loading || !builderSDKLoaded) {
    return (
      <div className="w-[414px] h-[896px] mx-auto bg-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-monumental-purple font-manrope">
            {!builderSDKLoaded ? 'Loading Builder.io SDK...' : 'Loading content...'}
          </p>
        </div>
      </div>
    );
  ...
  // Render the content using Builder.io SDK
  return (
    <div className="builder-renderer w-[414px] h-[896px] mx-auto overflow-hidden relative">
      {/* Fallback to JSON display if the Builder component doesn't render */}
      <div className="absolute inset-0 bg-white">
        <div className="p-4">
          <h3 className="text-lg font-medium text-monumental-purple mb-2 font-klasik">
            Content Data (Fallback)
          </h3>
          <div className="bg-gray-100 p-4 rounded-lg overflow-auto h-[800px]">
            <pre className="text-xs text-monumental-purple/70 font-mono">
      ...
      >
        {/* This will be replaced by the Builder.io content */}
