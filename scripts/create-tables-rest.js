...
// Función para ejecutar el script SQL a través de la API REST
async function createWellnessTables() {
  try {
    console.log('Iniciando creación de tablas para módulos de bienestar y productividad...');

    // Ejecutar el script SQL a través de la API REST
    try {
      const response = await axios.post(`${supabaseUrl}/rest/v1/rpc/pgclient`,
      ...
      console.log('Tablas creadas exitosamente:', data);
    } catch (error) {
      console.error('Error al ejecutar el script SQL:', error.response ? error.response.data : error.message);
      return;
    }
    console.log('Las tablas para los módulos de bienestar y productividad han sido creadas correctamente.');
  } catch (error) {
